...
Score every category from a FIXED yes/no checklist with a fixed denominator — never from invented, self-capped signal tallies. This is the CheckEval pattern (decompose into independent binary checks, roll up mechanically): reproducible run-to-run because the denominator does not float, and debuggable because you can see exactly which checks failed.
